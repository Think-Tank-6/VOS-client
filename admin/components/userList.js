...
const UserList = ({ navigation }) => {
  ...
  const fetchUsers = (page) => {
    ...
    fetch(url)
      ...
      .then(data => {
        setFilteredMemberList(data.users);
  ...
  const renderItem = ({ item }) => (
    ...
    <TouchableOpacity 
      ...
      <View>
        <CheckBox
        value={selectedUsers.includes(item.user_id)}
        onValueChange={() => handleSelectUser(item.user_id)}
        tintColors={{ true: 'black', false: 'white' }}
        />
      </View>
        <Text style={[styles.itemText, styles.marginL]}>{item.user_id}</Text>
        <Text style={styles.itemText}>{item.name}</Text>
        <Text style={styles.itemText}>{item.phone}</Text>
        <Text style={[styles.itemText]}>{formatDate(item.created_at)}</Text>
        <Text style={[styles.itemText, styles.center]}>0</Text>
  ...
  return (
    <View style={styles.memberListContainer}>
      {/* serach */}
      <Text style={styles.memberListTitle}>User List</Text>
      ...
      <View style={styles.memberListItemHeader}>
        ...
        <Text style={styles.headerText}>Chat Count</Text>
      </View>
      <FlatList
